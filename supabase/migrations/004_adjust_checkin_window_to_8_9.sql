...
declare
  ...
  business_day_of_week int := extract(dow from timezone('Africa/Cairo', now()))::int;
  computed_status text := case
    when business_day_of_week in (5, 6) then 'present'
    when timezone('Africa/Cairo', now())::time > time '09:00' then 'late'
    else 'present'
  end;
begin
